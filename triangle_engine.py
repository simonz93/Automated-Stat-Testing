...
# Default region / sub-column applied to ALL shapes unless overridden below.
...
def find_wave_columns(ws, region, sub_col):
    """Locate the Wave 4 / Wave 5 column pair matching region + sub_col."""
    match_set = {region.strip(), sub_col.strip()}
    # Wave label row — find the row containing 'Wave 4' / 'Wave 5'
    wave_row = None
    ...
    if not w4 or not w5:
        sys.exit(
            f"ERROR: No Wave 4 / Wave 5 column pair matching '{region}' > '{sub_col}'"
        )
...
def load_excel_data(ws, log=print):
    """Build section data for every unique (region, sub_col) pair in use."""
    all_sections = find_all_sections(ws)

    # Always have the default column pair
    col_specs = {(DEFAULT_REGION, DEFAULT_SUB_COLUMN)}
    for _, _, region, sub_col in SHAPE_OVERRIDES:
        col_specs.add((region, sub_col))

    data_by_col = {}
    for region, sub_col in col_specs:
        w4, w5, lw4, lw5 = find_wave_columns(ws, region, sub_col)
        tag = "" if (region, sub_col) == (DEFAULT_REGION, DEFAULT_SUB_COLUMN) else " [override]"
        log(
            f"  cols -> W4=col{w4}('{lw4}') W5=col{w5}('{lw5}') | "
            f"'{region}' > '{sub_col}'{tag}"
        )
        data_by_col[(region, sub_col)] = build_section_data(ws, all_sections, w4, w5, lw4, lw5)
...
def process_shape(slide_num, slide, shape, spec, section_data, region, sub_col, report_rows):
    """Dispatch one shape to chart/table/textbox handling. Returns updates count."""
    col_label = f"{region} > {sub_col}"
...
def scan_shapes(prs):
    """Return list of (slide_num, shape, region, sub_col)."""
    override_map = {(s, n): (r, c) for s, n, r, c in SHAPE_OVERRIDES}
    out = []
    for slide_num, slide in enumerate(prs.slides, 1):
        ...
        for shape in slide.shapes:
            if not shape.name.startswith(SHAPE_PREFIX):
                continue
            key = (slide_num, shape.name)
            region, sub_col = override_map.get(key, (DEFAULT_REGION, DEFAULT_SUB_COLUMN))
            out.append((slide_num, shape, region, sub_col))
    return out
...
def build_report(report_rows):
    ...
    banner.value = (
        ...
        f"{tri} triangles applied | Default: {DEFAULT_REGION} > {DEFAULT_SUB_COLUMN} | "
        f"Sheet: {EXCEL_SHEET}"
    )
